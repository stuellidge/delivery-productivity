...
export default class IncidentsSeeder extends BaseSeeder {
  ...
  async run() {
    ...
    const incidents: [string, number, number, string, string, string][] = [
      ['INC-2024-001', 82, 28, 'high', 'payments-api', 'Elevated 5xx error rate on /checkout endpoint'],
      ['INC-2024-002', 75, 65, 'critical', 'payments-api', 'Payment gateway timeout — downstream PSP unreachable'],
      ['INC-2024-003', 68, 42, 'medium', 'payments-worker', 'Refund processing queue backup — consumer lag spike'],
      ['INC-2024-004', 55, 18, 'low', 'search-api', 'Elevated p99 latency on /search after index rebuild'],
      ['INC-2024-005', 48, 95, 'high', 'payments-api', 'Cascade failure from database connection pool exhaustion'],
      ['INC-2024-006', 35, 32, 'medium', 'search-api', 'Personalisation service returning stale recommendations'],
      ['INC-2024-007', 20, 55, 'high', 'payments-worker', 'Idempotency key collision causing duplicate charges'],
      ['INC-2024-008', 8, 22, 'medium', 'payments-api', 'Auth token validation errors after certificate rotation'],
    ]
