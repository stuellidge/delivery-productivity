  ...
  async run() {
    ...
    for (const [ticketId, daysAgo, cycleTimeDays, storyPoints, sprintIdx] of payCompletedTickets) {
      ...
      if (!existingEvent) {
        ...
        await WorkItemEvent.createMany([
          {
            source: 'jira', ticketId, eventType: 'created', deliveryStreamId: payments.id,
            toStage: 'backlog', fromStage: null, storyPoints, sprintId: sprint?.id ?? null,
            ticketType: storyPoints >= 5 ? 'Story' : 'Task',
            eventTimestamp: startedAt.minus({ days: 1 }), ...rcvd,
          },
          {
            source: 'jira', ticketId, eventType: 'transitioned', deliveryStreamId: payments.id,
            fromStage: 'backlog', toStage: 'dev', storyPoints,
            eventTimestamp: startedAt, ...rcvd,
          },
          {
            source: 'jira', ticketId, eventType: 'transitioned', deliveryStreamId: payments.id,
            fromStage: 'dev', toStage: 'code_review', storyPoints,
            eventTimestamp: startedAt.plus({ days: cycleTimeDays * 0.6 }), ...rcvd,
          },
          {
            source: 'jira', ticketId, eventType: 'completed', deliveryStreamId: payments.id,
            fromStage: 'code_review', toStage: 'done', storyPoints,
            eventTimestamp: completedAt, ...rcvd,
          },
        ])
      }
    }
    ...
    for (const [ticketId, stage, daysIn] of payWipTickets) {
      ...
      if (!existingEvent) {
        ...
        await WorkItemEvent.createMany([
          {
            source: 'jira', ticketId, eventType: 'created', deliveryStreamId: payments.id,
            toStage: 'backlog', fromStage: null, storyPoints: 2,
          ...
          {
            source: 'jira', ticketId, eventType: 'transitioned', deliveryStreamId: payments.id,
            fromStage: 'backlog', toStage: stage as any, storyPoints: 2,
    ...
    for (const [ticketId, daysAgo, cycleTimeDays, storyPoints, sprintIdx] of srcCompletedTickets) {
      ...
      if (!existingEvent) {
        ...
        await WorkItemEvent.createMany([
          {
            source: 'jira', ticketId, eventType: 'created', deliveryStreamId: search.id,
            toStage: 'backlog', fromStage: null, storyPoints,
            ticketType: storyPoints >= 5 ? 'Story' : 'Task',
            eventTimestamp: startedAt.minus({ days: 1 }), ...rcvd,
          },
          {
            source: 'jira', ticketId, eventType: 'transitioned', deliveryStreamId: search.id,
            fromStage: 'backlog', toStage: 'dev', storyPoints,
            eventTimestamp: startedAt, ...rcvd,
          },
          {
            source: 'jira', ticketId, eventType: 'transitioned', deliveryStreamId: search.id,
            fromStage: 'dev', toStage: 'code_review', storyPoints,
            eventTimestamp: startedAt.plus({ days: cycleTimeDays * 0.65 }), ...rcvd,
          },
          {
            source: 'jira', ticketId, eventType: 'completed', deliveryStreamId: search.id,
            fromStage: 'code_review', toStage: 'done', storyPoints,
            eventTimestamp: completedAt, ...rcvd,
          },
        ])
    ...
    for (const [ticketId, stage, daysIn] of srcWipTickets) {
      ...
      if (!existingEvent) {
        ...
        await WorkItemEvent.createMany([
          {
            source: 'jira', ticketId, eventType: 'created', deliveryStreamId: search.id,
            toStage: 'backlog', fromStage: null, storyPoints: 3,
          ...
          {
            source: 'jira', ticketId, eventType: 'transitioned', deliveryStreamId: search.id,
            fromStage: 'backlog', toStage: stage as any, storyPoints: 3,
            eventTimestamp: startedAt,
            receivedAt: startedAt,
            sprintId: srcActiveSprint?.id ?? null,
          },
